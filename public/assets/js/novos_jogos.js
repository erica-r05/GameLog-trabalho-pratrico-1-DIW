...
window.excluirItem = async function(itemId) {
    ...
    try {
        ...
        if (responseJogos.ok && responseDetalhes.ok) {
            ...
            const errorDataJogos = await responseJogos.json().catch(() => ({}));
            const errorDataDetalhes = await responseDetalhes.json().catch(() => ({}));
            console.error('Erro ao excluir jogo:', responseJogos.status, errorDataJogos, responseDetalhes.status, errorDataDetalhes);
